...
def process_file(file_path):
    """
    Extrai o conteúdo do resumo via Langflow.
    ...
    try:
        ...
        try:
            # Tenta extrair a mensagem de texto (caminho comum no Langflow)
            # Se a estrutura for diferente do analyzer, ajustamos aqui.
            # Baseado na estrutura do analyzer: outputs > 0 > outputs > 0 > results > text > data > text
            # Mas vamos tentar uma navegação mais resiliente primeiro.
            ...
            try:
                ...
                # Se não for JSON, limpa possíveis tags markdown se houver
...
def main():
    ...
    print(f"Encontrados {len(api_files)} arquivos para processar no fluxo de RESUMO. Lote: {BATCH_SIZE}")
    
    # Process with progress bar and batch control
    with tqdm(total=len(api_files), desc="Resumindo Artigos") as pbar:
