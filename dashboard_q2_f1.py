...
from q2_fase1 import run_process_batch_mode, DEFAULT_API_URL

# Configuração da Página
st.set_page_config(page_title="RSL Processor Q2-F1: Ética e Regulação", layout="wide", page_icon="⚖️")
...
# Estado da Sessão
if 'logs_batch_q2' not in st.session_state:
    st.session_state.logs_batch_q2 = []
if 'is_running_q2' not in st.session_state:
    st.session_state.is_running_q2 = False

# Sidebar de Configurações
st.sidebar.title("⚙️ Configurações Q2")
st.sidebar.markdown("Questão 2: Ética e Regulação")
api_url = st.sidebar.text_input("Endpoint do Langflow (Q2)", value=DEFAULT_API_URL)
...
# Verificação de resultados Fase 1 Q2
output_f1_folder = "./q2-f1/"
f1_files = list(Path(output_f1_folder).glob("batch*.json")) if Path(output_f1_folder).exists() else []
total_f1 = len(f1_files)
st.sidebar.markdown(f"**Batches processados (Q2-F1):** {total_f1}")

# Interface Principal
st.title("⚖️ RSL Batch Processor - Q2 Fase 1")
st.markdown("Análise de **Ética e Regulação** em Lote utilizando Langflow")
...
with col3:
    st.metric("Status", "Parado" if not st.session_state.is_running_q2 else "Processando...")

start_btn = st.button("▶️ Executar Q2 - Fase 1", disabled=st.session_state.is_running_q2 or total_files == 0)
...
def update_progress(current, total, batch_ids):
    ...
    st.session_state.logs_batch_q2.insert(0, log_entry)

# Lógica de Execução
if start_btn:
    st.session_state.is_running_q2 = True
    st.session_state.logs_batch_q2 = []
    
    st.info("Iniciando o processamento Q2... Aguarde.")
    ...
    try:
        ...
        if not has_error:
            st.success("✅ Processamento Q2 - Fase 1 finalizado com sucesso!")
            st.balloons()
        else:
...
with log_container:
    st.subheader("📋 Fluxo de Processamento Q2")
    if st.session_state.logs_batch_q2:
        df_logs = pd.DataFrame(st.session_state.logs_batch_q2)
        st.table(df_logs)
    else:
        st.info("Aguardando início do processamento...")

# Aba de Visualização de Resultados
with st.expander("📂 Ver Arquivos Gerados em `./q2-f1/`"):
    output_folder = Path("./q2-f1/")
    if output_folder.exists():
        ...
        st.write("Pasta `./q2-f1/` não existe.")
