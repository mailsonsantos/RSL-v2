...
st.set_page_config(page_title="RSL Resumer Monitor", layout="wide", page_icon="📝")
...
# --- Estado da Sessão ---
if 'logs_resumo' not in st.session_state:
    st.session_state.logs_resumo = []
if 'processed_count_resumo' not in st.session_state:
    st.session_state.processed_count_resumo = 0
if 'failed_count_resumo' not in st.session_state:
    st.session_state.failed_count_resumo = 0
if 'is_processing_resumo' not in st.session_state:
    st.session_state.is_processing_resumo = False
if 'start_time_resumo' not in st.session_state:
    st.session_state.start_time_resumo = None

# --- Constantes do Langflow (Resumo) ---
API_URL = "http://localhost:7860/api/v1/run/080d8731-d1ec-4593-afff-401a59f115e5"
...
def process_file_logic(file_path, output_folder, api_key):
    ...
    logging.info(f"📝 Iniciando resumo: {file_name}")
    ...
    payload = {
        "output_type": "text",
        "input_type": "text",
        "input_value": "Gerar resumo do artigo",
    ...
    try:
        ...
        if isinstance(raw_text, str):
            ...
            try:
                processed_dict = json.loads(raw_text)
            except:
                processed_dict = {"resposta": raw_text}
        else:
        ...
        with open("erros_resumo.log", "a", encoding="utf-8") as f:
...
# --- Sidebar ---
st.sidebar.title("🛠️ Configurações de Resumo")
...
output_folder_val = st.sidebar.text_input("Pasta de Saída", value="resumos")
...
start_btn = st.sidebar.button("📝 Iniciar Resumos", 
                               disabled=st.session_state.is_processing_resumo or not API_KEY or total_files == 0)

# --- Cabeçalho ---
st.title("📝 Monitor de Resumo Automático (RSL)")
...
def update_metrics():
    processed = st.session_state.processed_count_resumo + st.session_state.failed_count_resumo
    remaining = total_files - processed
    m1.metric("Total PDFs", total_files)
    m2.metric("Processados", processed)
    m3.metric("Restantes", max(0, remaining))
    m4.metric("Sucessos", st.session_state.processed_count_resumo)
    m5.metric("Falhas", st.session_state.failed_count_resumo)
...
# --- Tabs ---
tab1, tab2, tab3 = st.tabs(["📊 Monitoramento", "🔍 Ver Resumos", "⚠️ Erros"])

with tab1:
    log_container = st.empty()
    if st.session_state.logs_resumo:
        log_container.table(pd.DataFrame(st.session_state.logs_resumo))

with tab2:
    if output_folder_path.exists():
        json_files = list(output_folder_path.glob("*.json"))
        if json_files:
            selected_json = st.selectbox("Selecione um resumo", options=[f.name for f in json_files])
            if selected_json:
...
with tab3:
    if Path("erros_resumo.log").exists():
        with open("erros_resumo.log", "r", encoding="utf-8") as f:
            errors = f.readlines()
...
if start_btn:
    if not check_langflow_connectivity(API_URL):
        ...
        st.session_state.is_processing_resumo = True
        st.session_state.start_time_resumo = time.time()
        st.session_state.processed_count_resumo = 0
        st.session_state.failed_count_resumo = 0
        st.session_state.logs_resumo = []
        ...
        for i in range(0, total_files, batch_size):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=len(batch)) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    st.session_state.logs_resumo.insert(0, result)
                    if result["status"] == "✅ Sucesso":
                        st.session_state.processed_count_resumo += 1
                    else:
                        st.session_state.failed_count_resumo += 1
                    
                    update_metrics()
                    progress_bar.progress((st.session_state.processed_count_resumo + st.session_state.failed_count_resumo)/total_files)
                    log_container.table(pd.DataFrame(st.session_state.logs_resumo))
