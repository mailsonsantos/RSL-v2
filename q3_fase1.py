...
# Configuração Padrão para Questão 3
DEFAULT_API_URL = "http://localhost:7860/api/v1/run/dab99159-5703-4e67-8ba5-662cf23170b1"
...
def process_batch(batch, batch_number, api_url, output_folder, api_key):
    ...
    try:
        logging.info(f"Enviando Batch {batch_number} (IDs: {ids_in_batch}) para API Q3...")
        response = requests.post(api_url, json=payload, headers=headers, timeout=600)
        
        if response.status_code != 200:
            logging.error(f"❌ Erro na API Q3 (Status {response.status_code}): {response.text}")
...
def run_process_batch_mode_q3(batch_size=15, api_url=DEFAULT_API_URL, progress_callback=None):
    """
    ...
    logging.info(f"Iniciando processamento Q3 de {total_files} arquivos em {total_batches} batches.")
    ...
    for idx, batch in enumerate(batches, 1):
        ...
        if res["status"] == "error":
            logging.error(f"⚠️ Processamento Q3 interrompido devido a erro no Batch {idx}.")
            break
...
if __name__ == "__main__":
    # Execução via CLI simples
    run_process_batch_mode_q3()
